...
| `bootstrap-registry.ts` | create reference-registry `InstrumentConfiguration` for BTC/USDC/ETH and LP instruments, plus the `Registry.V2` registry and its `InstrumentConfig`s (required before any mint) |
