...
The exact registry-side surface this DEX assumes is enumerated in the
repository's [registry-prerequisites.md](./registry-prerequisites.md)
and includes:
...
- the published code includes runnable Daml packages, tests, and a local dev
  environment
- the repo clearly documents the token-standard and registry
