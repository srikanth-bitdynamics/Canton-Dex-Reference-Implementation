...
async function routeRequest(
  ...
  if (method === "GET" && path === "/v1/price-history") {
    ...
    // window has to be too -- a seconds-based bound is ~1000x too small and
    // silently admits every row ever recorded.
  ...
  if (method === "GET" && path === "/v1/stats/24h") {
    ...
    // no price, so keeping those rows only makes the series denser. Volume is
    // read from swaps alone -- depositing liquidity is not trading.
