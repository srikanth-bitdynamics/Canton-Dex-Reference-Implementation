...
async function main(): Promise<void> {
  ...
  const { url, close } = startHttpServer({
    ...
    // DEX-96: operator token gates all non-admin writes; fail-closed on
    // testnet (no DEX_DEV_OPEN bypass here).
    operatorToken: process.env.DEX_OPERATOR_API_TOKEN,
    devOpen: false,
    // DEX-97: wallet relay OFF unless explicitly enabled, with a party allowlist.
